...
class CanvasBackground {
  ...
  canvasSize() {
      ...
      this.ctx.scale = this.ctx.scale(this.dpr,this.dpr);
  ...
  generateBubbles() {
     ...
     for (let i = 0; i < BUBBLE_DENSITY; i++) {
        ...
        let newBubble = new Bubble(this.canvas);
        
        this.bubblesList.push(newBubble);
  ...
  animate() {
    ...
    // this.ctx.clearRect = this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
...
const back = new CanvasBackground("orb-canvas");
back.start();
